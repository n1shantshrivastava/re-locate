$(document).ready(function () {
    ...
    $(function () {
        ...
        displayObjectsFromDatabase('objectBox-frontend');
    });
...
//        console.log('displayObjects method called');
    ...
    $("#show_all-frontend").click(function (e) {
        $('.circleBox-frontend').hide();
        $('.objectBox-frontend').show();
        displayObjectsFromDatabase('objectBox-frontend');
    });
    ...
    function attachObject(objectId, object_image_html, object_url_html, object_url) {
        var $object = $('<div class="object draggableObjects draggableObjects-frontend"  id="' + object_url + '" >' + object_image_html + object_url_html + '</div>');
    ...
    $(".draggableObjects-frontend").draggable({
    ...
    $(".draggableObjects-frontend").bind('dragstart', onDragStart, false);

    $(".mainContainer-frontend").droppable({
        accept:".draggableObjects-frontend",
...
//            console.log(user_id);
            ...
            $.ajax({
                ...
                success:function (result) {
                    ...
                    if(resultDt['status'] == 'success'){
                        addCirclefrontend(ui.draggable, thisobject.id);
                        $("#errorDivfrontend").html();
                        $("#errorDivfrontend").html('<span class="success">'+resultDt['message']+'</span>');
                    }else{
                        $("#errorDivfrontend").html();
                        $("#errorDivfrontend").html('<span class="error">'+resultDt['message']+'</span>');
                    }
    ...
    $(".mainContainer-frontend").mouseover(function () {
        ...
        TweenLite.to($("#b" + containerId), 0.2, {css:{width:150, height:150, marginLeft:-20, marginTop:-20}, ease:Power2.easeOut, onComplete:function () {
            calculatePositionsfrontend(containerId);
        }
        });
    });

    $(".mainContainer-frontend").mouseleave(function () {
    ...
    $(".smallCircle-frontend").click(function () {
        var object;
        ...
        $(bigCircleId).children(resultCircleClass).each(function () {
            var context = $(this);
            object = $('<div class="object draggableObjects draggableObjects-frontend" >' + context.html() + '</div>');
            $('.circleBox-frontend').append(object).masonry('appended', object);
        });

        $('.draggableObjects-frontend:not(.ui-draggable)').draggable({
            cancel:"a.ui-icon",
            revert:"invalid",
            containment:"document",
            helper:"clone",
    ...
    $(".bigCircle-frontend").on('click', '.resultCircle-frontend', function () {
...
function addCirclefrontend($item, containerId) {

    var resultCircleClass = "r" + containerId;
    console.log('addCircleCalled')
    var div = $("<div class='resultCircle resultCircle-frontend " + resultCircleClass + "' >" + $item.html() + "</div>");
    $('#b' + containerId).append(div);
    calculatePositionsfrontend(containerId);
}

function calculatePositionsfrontend(containerId) {
